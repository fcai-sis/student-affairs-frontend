...
export default function ShowErrorsButton({
  mappingErrors,
}: ShowErrorsButtonProps) {
...
const MappingErrorsModal = ({
  isOpen,
  onCancel,
  errors,
}: MappingErrorsModalProps) => {
          ...
          <div className="flex flex-col gap-4 overflow-y-auto max-h-96">
            {
              ...
              errors.map(({ row, errors }) => (
                <div
                  key={row}
                  className="flex flex-col gap-2 bg-slate-100 p-2 rounded-lg"
                >
